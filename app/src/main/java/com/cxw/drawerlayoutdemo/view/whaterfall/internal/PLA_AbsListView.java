...
package com.cxw.drawerlayoutdemo.view.whaterfall.internal;
...
import com.cxw.drawerlayoutdemo.view.whaterfall.DebugUtil;
                   ...
                   ViewTreeObserver.OnTouchModeChangeListener
{
    ...
    /**
     * as中不能调用view中的这个initializeScrollbars方法
     * eclipse中能够调用，故只能用反射了
     * @param a
     */
    protected void initializeScrollbars(TypedArray a) {
        try {
            final Method initializeScrollbars = android.view.View.class.getDeclaredMethod(
    ...
    @Override protected int computeVerticalScrollOffset() {
        ...
        if (firstPosition >= 0 && childCount > 0) {
            if (mSmoothScrollbarEnabled) {
                ...
                int height = view.getHeight();
                if (height > 0) {
                    return Math.max(firstPosition * 100 - (top * 100) / height + (int) ((float) getScrollY() / getHeight() * mItemCount * 100),
                                    0);
                }
            } else {
                int index;
    ...
    @Override protected float getTopFadingEdgeStrength() {
        ...
        if (count == 0) {
            ...
            return top < getPaddingTop() ?
                   (float) -(top - getPaddingTop()) / fadeLength :
                   fadeEdge;
        }
    }
    ...
    @Override protected float getBottomFadingEdgeStrength() {
        ...
        if (count == 0) {
            ...
            return bottom > height - getPaddingBottom() ?
                   (float) (bottom - height + getPaddingBottom()) / fadeLength :
                   fadeEdge;
        }
    }
     ...
     */
    @SuppressWarnings("deprecation") View obtainView(int position, boolean[] isScrap) {
        isScrap[0] = false;
        View scrapView;
    ...
    void positionSelector(View sel) {
        ...
        positionSelector(selectorRect.left,
                         selectorRect.top,
                         selectorRect.right,
                         selectorRect.bottom);
    ...
    private void positionSelector(int l, int t, int r, int b) {
        mSelectorRect.set(l - mSelectionLeftPadding,
                          t - mSelectionTopPadding,
                          r + mSelectionRightPadding,
    ...
    @Override public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        final int touchMode = isInTouchMode() ?
                              TOUCH_MODE_ON :
                              TOUCH_MODE_OFF;
    ...
    @Override public boolean showContextMenuForChild(View originalView) {
        ...
        if (longPressPosition >= 0) {
            ...
            if (mOnItemLongClickListener != null) {
                handled = mOnItemLongClickListener.onItemLongClick(PLA_AbsListView.this,
                                                                   originalView,
                                                                   longPressPosition,
                                                                   longPressId);
            }
            ...
            implements Runnable
    {
        ...
        void start(int initialVelocity) {
            ...
            int initialY = initialVelocity < 0 ?
                           Integer.MAX_VALUE :
                           0;
            ...
            switch (mTouchMode) {
                ...
                case TOUCH_MODE_FLING: {
                    ...
                    if (delta > 0) {
                        ...
                        delta = Math.min(getHeight() - getPaddingBottom() - getPaddingTop() - 1,
                                         delta);
                    } else {
                        ...
                        delta = Math.max(-(getHeight() - getPaddingBottom() - getPaddingTop() - 1),
                                         delta);
                    }
     ...
     */
    @SuppressWarnings("deprecation") boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
        final int childCount = getChildCount();
        if (childCount == 0) {
            return true;
        }
    ...
    int findClosestMotionRow(int y) {
        ...
        return motionRow != INVALID_POSITION ?
               motionRow :
               mFirstPosition + childCount - 1;
    }
            ...
            extends ViewGroup.LayoutParams
    {
         ...
         */
        @ViewDebug.ExportedProperty(mapping = {@ViewDebug.IntToString(from = ITEM_VIEW_TYPE_IGNORE,
                                                                      to = "ITEM_VIEW_TYPE_IGNORE"),
                                               @ViewDebug.IntToString(from = ITEM_VIEW_TYPE_HEADER_OR_FOOTER,
    ...
    class RecycleBin {
        ...
        View getScrapView(int position) {
            ...
            for (int i = size - 1; i >= 0; --i) {
                final LayoutParams lp = (LayoutParams) scrapViews.get(i)
                                                                 .getLayoutParams();
         ...
         */
        @SuppressWarnings("deprecation") void scrapActiveViews() {
            final View[] activeViews = mActiveViews;
